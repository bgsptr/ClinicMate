...
export class CreateOutpatientMapper implements IMapper<CreateOutpatientDto, OutpatientEntity, ResponseDto> {
    mapFromDto(input: CreateOutpatientDto, outPatientId: string, visitDate: Date, role: Role, ...args: any): OutpatientEntity {
        return new OutpatientEntity(
            ...
            role === Role.ADMIN ? VerificationStatus.ACCEPTED : VerificationStatus.PENDING
        )
    }
