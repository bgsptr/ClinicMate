...
export class ScheduleControler {
  ...
  async createNewSchedule(@Body() createScheduleDto: CreateScheduleDto) {
    try {
      ...
      if (err instanceof CustomConflictError) {
  ...
  @Put(':schedule_id')
  async updateScheduleWithScheduleId(
    @Param() schedule_id: string,
    @Body() create_schedule_dto: CreateScheduleDto,
  ) {
  ...
  async findAllScheduleFromEachDoctor(
    @Param() params: { doctor_id: string },
    @FindEmail() email: string,
    @FetchRoles() role: Role,
  ) {
    console.log(email);
    return await this.scheduleDoctorIdUseCase.execute(
      params.doctor_id,
      email,
      role,
    );
